...
extern struct uart_port *msm_hs_get_uart_port(int port_index);
extern void msm_hs_set_mctrl(struct uart_port *uport,
				    unsigned int mctrl);
...
static int bluesleep_lpm_enable (int en)
{
	...
	if (!en) {
		...
		if (!has_lpm_enabled) {
			...
			bsi->uport = msm_hs_get_uart_port(BT_UART_PORT_ID);
